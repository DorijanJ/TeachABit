...
export const Profil = () => {
  ...
  const deleteRacun = async () => {
    ...
    const response = await requests.deleteWithLoading(`account/${username}`);
    if (response && response.message?.severity === "success") {
      if (isDeletingSelf) {
        navigate("/tecajevi");
      } else navigate(-1);
        ...
        >
          <Card sx={{ minWidth: 300, width: "100%" }}>
            ...
            >
              ...
              {globalStore.currentUser !== undefined && isCurrentUser && (
              ...
              >
                ...
                {user.korisnikStatusId === KorisnikStatus.Utisan && (
                  <MicOffIcon
                    sx={{
                      height: "25px",
                      width: "25px",
              ...
              >
                {globalStore.hasPermissions(LevelPristupa.Admin) &&
                selectedUloga !== "Admin" &&
                  ...
                  {(globalStore.currentUser?.id === user.id ||
                    (globalStore.hasPermissions(LevelPristupa.Admin) &&
                      user.roles?.find((x) => x.name === "Admin") ===
                        undefined)) && (
                    <DeleteButtonAndPrompt deleteRacun={deleteRacun} />
                  )}
                  {globalStore.hasPermissions(LevelPristupa.Moderator) &&
                    user.roles?.find(
                      (x) => x.levelPristupa >= LevelPristupa.Moderator
                    ) === undefined && (
                      <Button
                        sx={{ height: "30px" }}
                        onClick={() => {
                          if (!username) return;
                          if (user.korisnikStatusId === KorisnikStatus.Utisan)
                            OdTisajKorisnika(username);
                          else UtisajKorisnika(username);
                        }}
                        variant="contained"
                      >
                        {user.korisnikStatusId === KorisnikStatus.Utisan
