...
public class RadioniceController(IRadioniceService radioniceService) : BaseController
{
    ...
    [AllowAnonymous]
    [HttpGet]
    public async Task<IActionResult> GetRadionicaList(string? search = null, string? vlasnikUsername = null, bool samoNadolazece = true)
    {
        var result = await _radioniceService.GetRadionicaList(search, vlasnikUsername, samoNadolazece);
